...
/**
 * Amazon Orders Page
 * Displays orders from "New Amazon Store" channel (Store ID: 2895995)
 */
export default function AmazonOrders() {
  const [, setLocation] = useLocation();
  const [searchQuery, setSearchQuery] = useState("");

  // Fetch orders filtered by Amazon store
  ...
  return (
    <div className="container py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Amazon Orders</h1>
          <p className="text-muted-foreground">
            Orders from New Amazon Store (Store ID: 2895995)
          </p>
        </div>
        <Button
          onClick={() => refetch()}
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <h3 className="text-lg font-semibold mb-2">No Amazon Orders Found</h3>
              <p className="text-sm text-muted-foreground max-w-md">
                {searchQuery
                  ? "No orders match your search criteria. Try adjusting your search."
                  : "No orders from the New Amazon Store channel yet."}
              </p>
            </div>
          ) : (
