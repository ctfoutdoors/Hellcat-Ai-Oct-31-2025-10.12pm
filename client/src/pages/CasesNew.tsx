...
import { Plus, Download, Sparkles, Loader2, Mail } from "lucide-react";
...
export default function Cases() {
  ...
  const sendBulkEmailMutation = trpc.cases.sendBulkEmail.useMutation();
  ...
  const handleBulkEmail = async (subject: string, body: string) => {
    try {
      ...
      if (failCount === 0) {
        toast.success(`Emails sent successfully to ${successCount} case(s)`);
      } else {
        toast.warning(`Sent to ${successCount} case(s), failed for ${failCount}`);
      }
    } catch (error: any) {
  ...
  return (
    <DashboardLayout>
      ...
      <BulkEmailDialog
        ...
        selectedCaseNumbers={cases?.filter(c => selectedCases.has(c.id)).map(c => c.caseNumber) || []}
