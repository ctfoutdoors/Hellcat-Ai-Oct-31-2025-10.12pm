...
import { letterPatternsRouter } from "./routers/letterPatterns";
import { caseTemplatesRouter } from "./routers/caseTemplates";
import { remindersRouter } from './routers/reminders';
import { dataExportRouter } from './routers/dataExport';
import { monitoringRouter } from './routers/monitoring';
import { googleCalendarRouter } from './routers/googleCalendar';
import { clipboardRouter } from './routers/clipboard';
import { screenshotOCRRouter } from './routers/screenshotOCR';
import { documentCategorizationRouter } from './routers/documentCategorization';
import { versionControlRouter } from './routers/versionControl';
import { exportPresetsRouter } from './routers/exportPresets';
import { purchaseOrdersRouter } from './routers/purchaseOrders';
import { receivingRouter } from './routers/receiving';
import { inventoryRouter } from './routers/inventory';
import { skuAliasesRouter } from './routers/skuAliases';
import { crmEmailRouter } from './routers/crmEmail';
import { crmDocumentsRouter } from './routers/crmDocuments';
import { gmailMonitoringRouter } from './routers/gmailMonitoring';
import { voiceCommandsRouter } from './routers/voiceCommands';
...
export const appRouter = router({
  ...
  letterPatterns: letterPatternsRouter,
  ...
  screenshotOCR: screenshotOCRRouter,
  documentCategorization: documentCategorizationRouter,
  versionControl: versionControlRouter,
  exportPresets: exportPresetsRouter,
  purchaseOrders: purchaseOrdersRouter,
  receiving: receivingRouter,
  inventory: inventoryRouter,
  skuAliases: skuAliasesRouter,
  crmEmail: crmEmailRouter,
  crmDocuments: crmDocumentsRouter,
  gmailMonitoring: gmailMonitoringRouter,
  ...
  cases: router({
    ...
    sendBulkEmail: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        const { sendBulkEmails } = await import('./services/bulkEmailService');
        ...
        for (const result of results) {
          if (result.success) {
            await db.createActivityLog({
              ...
              description: `Bulk email sent: ${input.subject}`,
            });
          }
        }
    ...
    bulkUpdateStatus: protectedProcedure
      .input(z.object({
        caseIds: z.array(z.number()),
        status: z.enum(["DRAFT", "FILED", "AWAITING_RESPONSE", "RESOLVED", "CLOSED", "REJECTED"]),
      }))
      .mutation(async ({ input, ctx }) => {
        const { BulkActionsHistoryService } = await import('./services/bulkActionsHistory');
        
        // Record action for undo
        const actionId = await BulkActionsHistoryService.recordAction(
          'status_change',
          input.caseIds,
        ...
        for (const caseId of input.caseIds) {
          await db.updateCase(caseId, { status: input.status });
          await db.createActivityLog({
            caseId,
            userId: ctx.user.id,
            actionType: 'STATUS_CHANGED',
            description: `Status changed to ${input.status} (bulk action)`,
          });
        }
        
        return { success: true, actionId, casesUpdated: input.caseIds.length };
      }),
    ...
    undoBulkAction: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        const result = await BulkActionsHistoryService.undoAction(input.actionId);
        
        return result;
      }),
    
    getRecentBulkActions: protectedProcedure
      .input(z.object({ limit: z.number().default(10) }))
      .query(async ({ input }) => {
        const { BulkActionsHistoryService } = await import('./services/bulkActionsHistory');
        
        const actions = BulkActionsHistoryService.getRecentActions(input.limit);
        return { actions };
      }),
  }),
