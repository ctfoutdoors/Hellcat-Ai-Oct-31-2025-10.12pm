...
export const evidencePackageRouter = router({
  ...
  /**
   * Build evidence package as compiled PDF
   */
  buildPDF: publicProcedure
    .input(z.object({ caseId: z.number() }))
    .mutation(async ({ input }) => {
      const pdfBuffer = await EvidencePackageService.buildEvidencePackagePDF(input.caseId);
      ...
      return {
        success: true,
        filename: `evidence-package-case-${input.caseId}.pdf`,
        data: base64,
