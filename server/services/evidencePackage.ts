...
export class EvidencePackageService {
  ...
  /**
   * Build evidence package as a single compiled PDF
   */
  static async buildEvidencePackagePDF(caseId: number): Promise<Buffer> {
    const db = await getDb();
    if (!db) throw new Error("Database not available");
    ...
    doc.moveDown();
    doc.fontSize(16).text(`Case #${caseData.caseNumber}`, { align: 'center' });
    doc.moveDown(2);
    doc.fontSize(12);
    doc.text(`Carrier: ${caseData.carrier}`);
    doc.text(`Tracking: ${caseData.trackingNumber}`);
    doc.text(`Dispute Amount: $${parseFloat(caseData.disputeAmount || '0').toFixed(2)}`);
    doc.moveDown();
    doc.text(`Prepared: ${new Date().toLocaleDateString()}`);
    ...
    doc.fontSize(11);
    let pageNum = 3;
    doc.text(`1. Cover Letter ............................ Page ${pageNum++}`);
    doc.text(`2. Case Summary ............................ Page ${pageNum++}`);
    if (certification) {
      doc.text(`3. Manufacturer Certification .............. Page ${pageNum++}`);
    }
    doc.text(`${certification ? '4' : '3'}. Supporting Documents ..................... Page ${pageNum}`);
    ...
    doc.fontSize(11);
    const caseSummary = this.generateCaseSummary(caseData);
    doc.text(caseSummary);
    
    // Certification
    if (certification) {
      doc.addPage();
      doc.fontSize(14).text('MANUFACTURER CERTIFICATION', { underline: true });
      doc.moveDown();
      doc.fontSize(11);
      const certDoc = this.generateCertificationDocument(certification);
      doc.text(certDoc);
    ...
    if (caseData.attachments.length > 0) {
      doc.text('The following documents are included as supporting evidence:');
      doc.moveDown();
      caseData.attachments.forEach((att, idx) => {
        doc.text(`${idx + 1}. ${att.filename || 'Attachment'} (${att.type || 'document'})`);
      });
      doc.moveDown();
      doc.text('Note: Original files are available in the ZIP package for full resolution.');
    } else {
      doc.text('No additional supporting documents attached.');
    }
